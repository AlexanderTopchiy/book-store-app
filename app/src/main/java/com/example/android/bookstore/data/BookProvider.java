...
public class BookProvider extends ContentProvider{
    ...
    private Uri insertBook(Uri uri, ContentValues values) {
        ...
        if (bookName == null) {
            throw new IllegalArgumentException("Book requires a book name");
        }

        // If the price is provided, check that it's greater than or equal to 0 cents
        Integer price = values.getAsInteger(BookEntry.COLUMN_BOOK_PRICE);
        if (price != null && price < 0) {
            throw new IllegalArgumentException("Book requires valid price");
        }

        // If the quantity is provided, check that it's greater than or equal to 0 pieces
    ...
    private int updateBook(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // check that the book name value is not null.
